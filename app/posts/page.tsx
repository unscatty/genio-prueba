...
import NewPostCard from "@/components/posts/new-post-card";
...
export default async function Page({
  ...
  const authors = [] as Awaited<ReturnType<typeof getAllAuthors>>;
  // const tags = await getAllTags();
  const tags = [] as Awaited<ReturnType<typeof getAllTags>>;
  ...
  const paginatedPosts = posts.filter((post => !!post))
      ...
      <Container>
        ...
        {paginatedPosts.length > 0 ? (
          <div className="grid md:grid-cols-3 gap-4 z-0">
            {paginatedPosts.map((post: WPPost) => (
            ...
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
                  ...
                  href={`/posts?page=${Math.max(page - 1, 1)}${category ? `&category=${category}` : ""
              ...
              <PaginationItem>
                <PaginationNext
                  ...
                  href={`/posts?page=${Math.min(page + 1, totalPages)}${category ? `&category=${category}` : ""
                    }${author ? `&author=${author}` : ""}${tag ? `&tag=${tag}` : ""
                    }`}
                />
              </PaginationItem>
